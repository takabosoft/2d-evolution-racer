    ...
    private buildCoursePanel(course: Course): JQuery {
        ...
        return $(`<div class="course-panel">`).append(
            ...
            $(`<div class="info">`).append(
                ...
                $(`<button class="play-button">`).text("遺伝的アルゴリズムの様子を見る").on("click", e => this.sceneController.changeScene(new GeneticGameScene(this.sceneController, course.courseId))),
                $(`<button class="play-button">`).text("自分で操作する").on("click", e => this.sceneController.changeScene(new GameScene(this.sceneController, course.courseId))),
            )
        )
    }
}